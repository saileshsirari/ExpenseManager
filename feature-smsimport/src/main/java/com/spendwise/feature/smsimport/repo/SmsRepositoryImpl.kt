...
class SmsRepositoryImpl @Inject constructor(private val db: AppDatabase): SmsRepository {
    override suspend fun importAll(resolverProvider: () -> ContentResolver): Flow<List<SmsEntity>> = flow {
    ...
    override suspend fun saveManual(sender: String, body: String, timestamp: Long) {
        ...
        db.smsDao().insert(SmsEntity(sender = sender, body = body, timestamp = timestamp, amount = parsed?.amount ?: 0.0, merchant = parsed?.merchant, type = parsed?.type))
    }
}
